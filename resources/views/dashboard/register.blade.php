...
<title>Renewal Register — UNITELO</title>
...
.rg-h1{font-size:1.55rem;font-weight:900;letter-spacing:-.04em;color:var(--db-text);margin-bottom:20px}

.rg-list{border:1px solid var(--db-border);border-radius:16px;overflow:hidden}
.rg-table{width:100%;border-collapse:collapse;font-size:12.5px}
.rg-table thead th{text-align:left;padding:12px 18px;font-size:11px;font-weight:600;color:var(--db-text-muted);border-bottom:1px solid var(--db-border);white-space:nowrap}
.rg-table tbody tr{border-bottom:1px solid var(--db-border)}
.rg-table tbody tr:last-child{border-bottom:none}
.rg-table tbody tr:hover{background:var(--db-chip)}
.rg-table td{padding:11px 18px;vertical-align:top}
.rg-empty{padding:48px 18px;text-align:center;color:var(--db-text-muted);font-size:13.5px}
.rg-tx-id{font-family:monospace;color:var(--db-text-muted);font-size:11.5px}
.rg-badge{font-size:11px;padding:2px 9px;border-radius:99px;background:var(--db-chip);color:var(--db-text);font-weight:600}
...
@media(max-width:1024px){
  ...
  .rg-list{overflow-x:auto}
  .rg-table{min-width:820px}
}
</style>
<script>
(function () {
...
<div class="ob-page">
  ...
  <aside class="ob-sidebar">
    ...
    <div class="ob-security">
      ...
      <p>Every renewal your workers track, in one place.</p>
    </div>
  </aside>
  ...
  <main class="mem-main">
    <div class="mem-wrap">
      ...
      <div class="rg-h1">Renewal Register</div>

      <div class="rg-list">
        <table class="rg-table">
          <thead>
            <tr>
              <th>TX</th><th>Category</th><th>Asset</th><th>Client</th><th>Contact</th>
              <th>Due Date</th><th>Priority</th><th>Status</th><th>Decision</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr>
              <td><span class="rg-tx-id">{{ $row->tx_id }}</span></td>
              <td style="color:var(--db-text)">{{ $row->category }}</td>
              <td style="color:var(--db-text-muted)">{{ $row->asset }}</td>
              <td style="color:var(--db-text-muted)">{{ $row->client }}</td>
              <td style="color:var(--db-text-muted);font-size:11.5px">{{ $row->contact }}</td>
              <td>
                @if($days !== null)
                <span style="color:{{ $days <= 0 ? '#ef4444' : ($days <= 15 ? '#f59e0b' : 'var(--db-text-muted)') }}">
              ...
              <td><span style="color:{{ $row->priority === 'High' || $row->priority === 'Critical' ? '#f59e0b' : 'var(--db-text-muted)' }}">{{ $row->priority }}</span></td>
              <td><span class="rg-badge">{{ $row->status }}</span></td>
              <td style="color:var(--db-text-muted)">{{ $row->human_decision ?? '—' }}</td>
            </tr>
            @empty
            <tr><td colspan="9" class="rg-empty">No entries yet.</td></tr>
            @endforelse
          </tbody>
        </table>
      </div>
