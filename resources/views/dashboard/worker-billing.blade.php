...
<title>{{ $dep->name }} · Billing — UNITELO</title>
...
<div class="ob-page">
  ...
  <main class="mem-main">
    <div class="mem-wrap">
      ...
      <a href="{{ route('app.billing') }}" style="font-size:12px;color:var(--db-text-muted);text-decoration:none;display:inline-block;margin-bottom:8px">← Billing</a>
      <div class="wb-h1">Billing — {{ $dep->name }}</div>
      <div class="wb-sub">Usage and cost breakdown for this worker — subscription and plan live on the main <a href="{{ route('app.billing') }}" style="color:var(--db-text-muted);text-decoration:underline">Billing</a> page.</div>
      ...
      <div class="wb-banner trial">
        ...
        <a href="{{ route('app.billing.checkout', $dep->id) }}" class="wb-btn">Upgrade →</a>
      </div>
      @elseif($isActiveSub)
      <div class="wb-banner active">
        ...
        <a href="{{ route('app.billing.portal') }}" class="wb-btn-secondary">Manage billing →</a>
      </div>
      @else
