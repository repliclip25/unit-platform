...
<title>{{ $dep->name }} · Connect — UNITELO</title>
...
$sidebarLinks = [
  ...
  ['Integrations', 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1', route('app.workers.connect',$dep->worker_slug), true],
...
<div class="ob-page">
  ...
  <aside class="ob-sidebar">
    ...
    <div class="ob-security">
      ...
      <p>UNITELO never stores your password — OAuth only.</p>
    </div>
  </aside>
  ...
  <main class="mem-main">
    <div class="mem-wrap">
      ...
      <div class="con-list">
        ...
        <div class="con-row">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div class="con-actions">
              <form method="POST" action="{{ route('app.workers.inboxes.rewatch', [$dep->id, $inbox->id]) }}">
                @csrf
                <button type="submit" class="con-btn {{ !$inbox->watch_active ? 'warn' : '' }}">↺ {{ $inbox->watch_active ? 'Renew watch' : 'Rewatch' }}</button>
              </form>
              <form method="POST" action="{{ route('app.workers.inboxes.disconnect', [$dep->id, $inbox->pivot_id]) }}" onsubmit="return confirm('Disconnect {{ $inbox->gmail_address }}?')">
                @csrf @method('DELETE')
        ...
        <div class="con-footer">
          @if($availableCredentials->isNotEmpty())
            ...
            <form method="POST" action="{{ route('app.workers.inboxes.connect', $dep->id) }}" class="con-connect-row">
              @csrf
      ...
      <div class="con-info">
        🔒 <strong>Secure OAuth 2.0.</strong> UNITELO never stores your password. Watch subscriptions expire every 7 days and are auto-renewed by the scheduler. You can disconnect at any time.
      </div>
