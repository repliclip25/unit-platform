...
$sidebarLinks = [
  ...
  ['Billing',      'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z', route('app.billing'), true],
  ...
  <main class="mem-main">
    <div class="mem-wrap">
      ...
      <div class="bill-header">
        ...
        <a href="{{ route('app.billing.portal') }}" class="mem-btn-secondary">Manage Payment →</a>
      </div>
      ...
      <div class="bill-card {{ $needsConversion && $processed > 0 ? 'highlight' : '' }}" id="dep-{{ $dep->id }}">
        ...
        <div class="bill-card-body">
          ...
          <div class="bill-status-box warn">
            ...
            <a href="{{ route('app.billing.portal') }}" class="mem-btn" style="background:#ef4444;color:#fff">Update Payment Method →</a>
          </div>
          @elseif($isCanceled)
          ...
          @elseif($isActive)
            ...
            @endif
            <div style="margin-top:12px"><a href="{{ route('app.billing.portal') }}" class="mem-btn-secondary">Manage Subscription →</a></div>
          @endif
          ...
          <div style="{{ ($isExhausted || $isCanceled) ? 'margin-top:16px' : 'margin-top:8px' }}">
            ...
            <div class="bill-plans">
              ...
              <div class="bill-plan {{ $isPro ? 'pro' : '' }}">
                ...
                @else
                <a href="{{ route('app.billing.checkout', $dep->id) }}?plan={{ $tier->plan_slug }}" class="bill-plan-cta {{ $isPro ? 'primary' : '' }}">Get {{ ucfirst($tier->plan_slug) }} →</a>
                @endif
              </div>
              @endforeach
            </div>
      ...
      <div class="bill-card" style="margin-top:8px">
        ...
        <div class="bill-inv-row">
          ...
          <div class="bill-inv-actions">
            ...
            @endif
            <a href="{{ route('app.billing.invoice', $invoice->id) }}" class="bill-inv-link">Download PDF →</a>
