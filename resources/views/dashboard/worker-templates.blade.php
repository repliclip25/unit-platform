...
<title>{{ $dep->name }} · Templates — UNITELO</title>
...
<div class="tpl-modal-overlay" id="add-modal">
  <div class="tpl-modal">
    ...
    <form method="POST" action="{{ route('app.workers.templates.store', $dep->id) }}">
      @csrf
      <div class="tpl-modal-body">
        <div class="mem-field-row">
...
function openEditModal(id, t) {
    ...
    document.getElementById('edit-form').action   = '/app/workers/' + WORKER_ID + '/templates/' + id;
...
async function customizeTemplate(defaultId) {
    ...
    try {
        const res = await fetch('/app/workers/' + WORKER_ID + '/templates/' + defaultId + '/fork', {
            method: 'POST',
