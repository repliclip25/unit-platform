...
{{-- Heading --}}
<div class="mb-6">
    <h1 class="text-2xl font-black text-white mb-3 leading-snug">What kind of renewals do you handle?</h1>
    <p class="text-gray-400 text-sm leading-relaxed">Not every renewal looks the same.</p>
    <div class="mt-2 space-y-0.5">
        @foreach(['Software licenses.', 'Insurance policies.', 'Domain names.', 'Business permits.'] as $eg)
        <p class="text-gray-500 text-sm">{{ $eg }}</p>
        @endforeach
    </div>
    <p class="text-gray-400 text-sm leading-relaxed mt-3">
        Choose the business that's closest to yours so Ava knows what to look for and how to communicate with your clients.
    </p>
</div>
...
<form method="POST" action="{{ route('onboarding.step.handle', 'persona') }}" x-data="{ selected: '{{ $current ?? '' }}' }">
    ...
    <p class="text-center text-xs text-gray-600 mt-3">You can change this from your worker settings later.</p>
</form>
@endif
