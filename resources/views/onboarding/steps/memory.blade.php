...
@if($hasMemory)
<div class="bg-green-500/10 border border-green-500/20 rounded-xl px-5 py-4 mb-4 flex items-center gap-3">
...
<div class="bg-gray-900 border border-gray-800 rounded-xl mb-5 overflow-hidden">
    <button type="button" onclick="this.nextElementSibling.classList.toggle('hidden');this.querySelector('svg').classList.toggle('rotate-180')"
        ...
        <span class="text-gray-400 text-xs font-semibold uppercase tracking-widest">Loaded data preview</span>
        <svg class="w-4 h-4 text-gray-600 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
    </button>
    <div class="hidden border-t border-gray-800 divide-y divide-gray-800/60">
        @foreach($sampleClients as $client)
        <div class="px-5 py-3">
            <div class="flex items-center gap-2 mb-1">
                <span class="text-white text-sm font-semibold">{{ $client->name }}</span>
                @if($client->industry)
                    <span class="text-gray-600 text-xs">· {{ $client->industry }}</span>
                @endif
            </div>
            @if($client->contacts->count())
                <p class="text-gray-600 text-xs mb-1">
                    {{ $client->contacts->pluck('name')->join(', ') }}
                    @if($client->contacts->first()?->email)
                        · <span class="text-gray-700">{{ $client->contacts->first()->email }}</span>
                    @endif
                </p>
            @endif
            @if($client->assets->count())
                <div class="flex flex-wrap gap-1 mt-1.5">
                    ...
                    <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-gray-800 text-gray-500">
                        {{ $asset->type }} · {{ $asset->name }}
                        @if($asset->renewal_date)
                            <span class="text-yellow-600">· {{ \Carbon\Carbon::parse($asset->renewal_date)->diffForHumans() }}</span>
                        @endif
                    </span>
                    @endforeach
                </div>
