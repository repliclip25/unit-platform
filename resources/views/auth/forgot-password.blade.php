...
<title>Reset password — UNITELO</title>
...
<div class="au-wrap">
  <div class="au-card">
    <div class="au-h1">Reset your password</div>
    <div class="au-sub">Enter your email and we'll send a reset link.</div>
    ...
    <form method="POST" action="{{ route('password.email') }}">
      ...
      <div class="au-field">
        ...
        <input class="au-input" id="email" type="email" name="email" value="{{ old('email') }}" required autofocus placeholder="you@company.com">
        @error('email')<div class="au-error">{{ $message }}</div>@enderror
      </div>
      <button type="submit" class="au-submit">Email Reset Link</button>
    </form>

    <div class="au-footer">Remember it? <a href="{{ route('login') }}">Back to sign in</a></div>
  </div>
</div>
