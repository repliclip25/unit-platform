...
use App\Platform\Services\Memory\MemoryCoverageCalculator;
use Illuminate\Support\Facades\DB;

class MemoryHealthService
{
    // Complete records needed across the tenant's deployed workers to reach healthy status.
    public const HEALTHY_THRESHOLD = MemoryCoverageCalculator::HEALTHY_THRESHOLD;
    ...
    public static function score(int $userId): array
    {
        ...
        return array_merge(
            MemoryCoverageCalculator::scoreClients($userId, $requirements, self::HEALTHY_THRESHOLD),
            ['needs_memory' => true]
        );
    }
