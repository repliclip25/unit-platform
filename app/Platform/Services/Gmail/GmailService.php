...
class GmailService
{
    ...
    public function __construct(?object $credential = null)
    {
        ...
        $rawToken = $credential?->refresh_token ?? config('services.gmail.refresh_token');
        // Tokens stored via GmailController are Laravel-encrypted; fall back to raw for .env tokens
        try {
            $refreshToken = \Illuminate\Support\Facades\Crypt::decryptString($rawToken);
        } catch (\Throwable) {
            $refreshToken = $rawToken; // legacy unencrypted token — accepted during migration window
        }
        $this->fromAddress = $credential?->gmail_address ?? config('services.gmail.address');
        $this->accessToken = $this->getAccessToken($refreshToken);

        // Stamp last-used timestamp for rotation auditing
        if ($credential?->id) {
