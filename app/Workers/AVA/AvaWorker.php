...
class AvaWorker implements WorkerContract
{
    ...
    public function employee(): array
    {
        return [
            ...
            'mission'     => "AVA is UNITELO's AI renewal coordinator — renewal management software that monitors your inbox and manages the full renewal lifecycle, with human approval built in.",
    ...
    // Derived from pipelineStages() rather than hand-maintained as a
    // separate array — the two used to drift apart silently (the exact
    // failure mode behind two bugs fixed this session: Read Email's
    // Configure card being invisible because its key didn't match, and
    // Draft Email's saved override never applying for the same reason).
    // Deriving from one source makes that class of bug structurally
    // impossible: there's only one place a stage's AI prompt lives now.
    public function prompts(): array
    {
        return collect($this->pipelineStages())
            ->filter(fn($stage) => $stage['uses_ai'] ?? false)
            ->map(fn($stage) => [
                'stage'         => $stage['key'],
                'label'         => $stage['label'],
                'uses_ai'       => true,
                'model'         => $stage['model']         ?? null,
                'system'        => $stage['system']        ?? null,
                'user'          => $stage['user']           ?? null,
                'output_format' => $stage['output_format']  ?? null,
                'output_shape'  => $stage['output_shape']   ?? null,
                'max_tokens'    => $stage['max_tokens']     ?? null,
            ])
            ->values()
            ->all();
    }
